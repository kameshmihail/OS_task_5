...
int main() {
    ...
    void *libHandle;
    libHandle = dlopen("lib.so", RTLD_LAZY);
    if (!libHandle) {
        fprintf(stderr, "%s\n", dlerror());
        return 1;
    }

    deque_alloc = dlsym(libHandle, "deque_alloc");
    deque_free = dlsym(libHandle, "deque_free");
    deque_is_empty = dlsym(libHandle, "deque_is_empty");
    deque_push_front = dlsym(libHandle, "deque_push_front");
    deque_push_back = dlsym(libHandle, "deque_push_back");
    deque_pop_front = dlsym(libHandle, "deque_pop_front");
    deque_peek_front = dlsym(libHandle, "deque_peek_front");
    deque_pop_back = dlsym(libHandle, "deque_pop_back");
    deque_peek_back = dlsym(libHandle, "deque_peek_back");
